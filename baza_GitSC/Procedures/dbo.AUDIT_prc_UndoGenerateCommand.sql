...
SET NOCOUNT OFF
declare @cmptlvl int
Select @cmptlvl = t1.cmptlevel from master.dbo.sysdatabases t1 where t1.[name]=DB_NAME();
...
'   ELSE ''
  ...
  BEGIN
	...
	begin
		if ((@cmptlvl > 70) and (@cmptlvl < 90)) begin 
			select @IdentityCount = Count(*) from syscolumns where id=object_id(@TabName) and @ColumnList like '%[' + name +']%';
		end
		else begin
